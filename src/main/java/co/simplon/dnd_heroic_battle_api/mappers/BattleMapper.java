...
public class BattleMapper {
	...
	public static List<BattleView> entitiesToBattleViews(List<Battle> battles) {
		return battles.stream()
				.map(b -> new BattleView(b.getBattleName(), new CampaignView(b.getCampaign().getCampaignName(), b.getCampaign().getCreationDate()))).toList();
	}

	public static BattleView entityToBattleView(Battle battle) {
		return new BattleView(battle.getBattleName(), new CampaignView(battle.getCampaign().getCampaignName(), battle.getCampaign().getCreationDate()));
	}
