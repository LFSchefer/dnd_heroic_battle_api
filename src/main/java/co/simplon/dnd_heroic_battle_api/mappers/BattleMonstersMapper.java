...
public final class BattleMonstersMapper {
    ...
    public static Set<BattleMonsterPreviewDto> setEntitiesToSetPreviewDto (Set<BattleMonster> battleMonsters) {
        return battleMonsters.stream().map( m -> new BattleMonsterPreviewDto(m.getBattleMonsterId(),m.getName(),m.getCurrentHitPoints(),m.getInitiative(),m.getMonster().getMonsterId())).collect(Collectors.toSet());
    }
}
